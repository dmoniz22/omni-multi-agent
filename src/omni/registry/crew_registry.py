...
class CrewRegistry:
    ...
    def register(self, crew_class: Type[BaseCrew]) -> None:
        ...
        self._crew_info[name] = {
            ...
            "input_schema": crew_class.input_schema.__name__
            if crew_class.input_schema
            else None,
            "output_schema": crew_class.output_schema.__name__
            if crew_class.output_schema
            else None,
            ...
            for py_file in dept_dir.glob("*.py"):
                ...
                try:
                    ...
                    for name, obj in inspect.getmembers(module, inspect.isclass):
                        if (
                            issubclass(obj, BaseCrew)
                            and obj is not BaseCrew
                            and obj.name
                    ...
                    logger.warning(
                        "Failed to import crew module", module=module_name, error=str(e)
                    )
                except Exception as e:
                    logger.error(
                        "Error discovering crew", module=module_name, error=str(e)
                    )
