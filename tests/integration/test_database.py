...
from omni.db.engine import close_engine, get_session, health_check
...
async def test_session_crud():
    ...
    async with get_session() as db_session:
        ...
        # Create
        session = await repo.create(user_id="test_user", metadata={"test": True})
        ...
        updated = await repo.update(
            session.id, status="completed", metadata={"test": True, "updated": True}
        )
...
async def test_task_crud():
    ...
    async with get_session() as db_session:
        ...
        # Create task
        task = await task_repo.create(session_id=session.id, original_task="Test task")
        ...
        updated = await task_repo.update(
            task.id, status="completed", final_response={"result": "success"}
        )
        assert updated is not None
        assert updated.status == "completed"
...
async def test_end_to_end_workflow():
    ...
    async with get_session() as db_session:
        ...
        session = await session_repo.create(
            user_id="test_user", metadata={"workflow": "test"}
        )

        # Create task
        task_repo = TaskRepository(db_session)
        task = await task_repo.create(
            session_id=session.id, original_task="Research AI trends"
        )
